...
public class Server {
            ...
            while (true) {
                ...
                if (newSocket != null) {
                    ...
                    try {
                        ...
                        System.out
                                .print("Couldn't initialize read/write buffer");
                        break;
                    }
                }
            }
            ...
            if (!input.isEmpty() && input.charAt(0) == '/') {
                ...
                case "listen":
                    if (inputCommand.length != 2)
                        throw new IOException(
                                "Wrong arguments number for command \"listen\"");
                    else {
                        System.out.println("Start to listening "
                                + inputCommand[1]);
                        serverThread = new Thread(new ServerListener(
                ...
                case "send":
                    if (inputCommand.length != 3)
                        throw new IOException(
                                "Wrong arguments number for command \"send\"");
                    else if (listeners.contains(new Listener(inputCommand[1])))
                        listeners
                                .get(listeners.indexOf(new Listener(
                                        inputCommand[1]))).send(
                                        new Message(MessageType.MESSAGE,
                                                "server", inputCommand[2]));
                    else
                        System.out.println("User " + inputCommand[1]
                                + " not found");
                    break;
                ...
                case "sendall":
                    if (inputCommand.length < 2)
                        throw new IOException(
                                "Wrong arguments number for command \"sendall\"");
                    else if (inputCommand.length == 2)
                        sendAll(new Message(MessageType.MESSAGE, "server",
                                inputCommand[1]), true);
                    else
                        sendAll(new Message(MessageType.MESSAGE, "server",
                                inputCommand[1] + " " + inputCommand[2]), true);
                    break;

                case "kill":
                    if (inputCommand.length != 2)
                        throw new IOException(
                                "Wrong arguments number for command \"kill\"");
                    else {
                        if (listeners.contains(new Listener(inputCommand[1]))) {
                            listeners.get(
                                    listeners.indexOf(new Listener(
                                            inputCommand[1]))).send(
    ...
    public void kill(String user) {
        listeners.get(listeners.indexOf(new Listener(user))).myThread
                .interrupt();
        listeners.get(listeners.indexOf(new Listener(user))).closeSocket();
        listeners.remove(new Listener(user));
    }
    ...
    private class Listener implements Runnable {
        ...
        public boolean equals(Object obj) {
            if (obj instanceof Listener)
                return nickname != null && ((Listener) obj).nickname != null
                        && nickname.equals(((Listener) obj).nickname);
            else
                return false;
        }
        ...
        public void run() {
            ...
            if (message != null)
                sendAll(new Message(MessageType.MESSAGE, "server", "user "
                        + message.getName() + " has disconnected"), true);
        }
    }
}
