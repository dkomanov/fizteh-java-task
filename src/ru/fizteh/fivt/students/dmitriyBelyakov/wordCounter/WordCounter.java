                    ...
                    if (mode == Mode.MINUS_W) {
                        ...
                        if (unique == ModeUnique.NONE) {
                            ...
                            for (String word : words) {
                                if (map.containsKey(word)) {
                                    map.put(word, map.get(word).add(new BigInteger("1")));
                                } else {
                                    map.put(word, new BigInteger("1"));
                                }
                            }
                        } else {
                            for (String word : words) {
                                if (map.containsKey(word.toLowerCase())) {
                                    map.put(word.toLowerCase(), map.get(word.toLowerCase()).add(new BigInteger("1")));
                                } else {
                                    map.put(word.toLowerCase(), new BigInteger("1"));
                                }
                            }
                        }
                    } else {
                        if (unique == ModeUnique.NONE) {
                            ...
                            if (map.containsKey(str)) {
                                map.put(str, map.get(str).add(new BigInteger("1")));
                            } else {
                                map.put(str, new BigInteger("1"));
                            }
                        } else {
                            if (map.containsKey(str.toLowerCase())) {
                                map.put(str.toLowerCase(), map.get(str.toLowerCase()).add(new BigInteger("1")));
                            } else {
                                map.put(str.toLowerCase(), new BigInteger("1"));
                            }
                        }
                    }
                }
