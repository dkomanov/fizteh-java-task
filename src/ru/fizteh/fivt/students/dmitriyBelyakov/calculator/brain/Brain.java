...
public class Brain {
     ...
     *            Second operand
     */
    private static BigInteger calculateOperation(char operation,
            BigInteger first, BigInteger second) throws RuntimeException {
        BigInteger tmp;
        switch (operation) {
        case '+':
        ...
        while (true) {
            ...
            if ((i < exp.length()) && Character.isDigit(currentChar)) {
                ...
                if (!operandIsEmpty) {
                    operandIsEmpty = true;
                    try {
                        interpreter.push(new BigInteger(exp.substring(
                                firstOperandPosition, i)));
                    } catch (Exception e) {
                        throw new RuntimeException(
                                "Some operand is very large.");
                    }
                }
                if (translator.empty()) {
                    break;
