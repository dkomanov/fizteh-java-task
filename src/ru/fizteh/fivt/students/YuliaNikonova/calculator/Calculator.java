	...
	public static void main(String [] args) {
		...
		try {
			...
			if (myExpressionOPNString.equals("")) {
				System.out.println("Error: empty expression, you should enter expression as argument");
			} else {
			...
			if (Character.isDigit(c)) {
				...
				if (c == '+' || c == '-' || c == '*' || c == '/') {
					if ((c == '+' || c== '-') 
						...
						&& ((i == 0 ||  myExpression.charAt(i-1) == '(')
						|| (i>1 && myOperators.contains(String.valueOf(myExpression.charAt(i-1))) 
							&& (Character.isDigit(myExpression.charAt(i-2)) || myExpression.charAt(i-2)==')'))
						)) {
						number += c;
						...
						while (!opnStack.isEmpty() 
								&& myOperators.contains(opnStack.get(opnStack.size()-1))
